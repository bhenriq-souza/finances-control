# Architecture Decision Records

Formato: Context · Decision · Consequences · Alternatives considered. ADRs aceitos são imutáveis — mudanças geram um novo ADR que supersede o anterior.

| ADR | Título | Status |
|---|---|---|
| [ADR-0000](ADR-0000-hosting.md) | Hospedagem no cluster K3s homelab | accepted |
| [ADR-0001](ADR-0001-frontend.md) | Stack do front-end | **open** |
| [ADR-0002](ADR-0002-backend.md) | Stack e repositório do back-end | accepted |
| [ADR-0003](ADR-0003-architecture-style.md) | Estilo arquitetural (monolito modular vs microservices) | **open** |
| [ADR-0004](ADR-0004-database.md) | Banco de dados: PostgreSQL + TypeORM | accepted |
| [ADR-0005](ADR-0005-queue.md) | Mensageria/filas | **open** |
| [ADR-0006](ADR-0006-auth.md) | Autenticação: Firebase Auth | **draft** |
